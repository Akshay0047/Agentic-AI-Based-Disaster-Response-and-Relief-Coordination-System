...
_engine = create_engine(
    ...
    poolclass=None,  # NullPool — no connection pooling, avoids cross-loop bugs
    echo=False,
)


def _session() -> OrmSession:
    """Open a new synchronous SQLAlchemy session on the NullPool engine."""
    return OrmSession(bind=_engine)
...
def _classify_risk(tool_name: str, args: Dict[str, Any]) -> str:
    ...
    - Reading/lookup tools → low-risk
    - Updating request status → medium-risk (depends on new_status)
    """
    high_risk = {
    ...
    if tool_name in medium_risk:
        if args.get("new_status") in ("resolved", "cancelled"):
            return "low"
        return "medium"
    return "low"
...
def execute_tool(tool_name: str, args: Dict[str, Any]) -> ExecutorResult:
    ...
    try:
        if tool_name == "get_emergency_request":
            req = db.get(EmergencyRequest, args["request_id"])
            if not req:
            ...
            vol = db.get(Volunteer, args["volunteer_id"])
            if not vol:
                return {"status": "blocked", "tool": tool_name, "args": args,
                        "error": f"Volunteer {args['volunteer_id']} not found", "result": None}
            u = db.get(User, vol.user_id)
            return {"status": "executed", "tool": tool_name, "args": args,
            ...
            shel = db.get(Shelter, args["shelter_id"])
            if not shel:
                return {"status": "blocked", "tool": tool_name, "args": args,
            ...
            if not shel:
                return {"status": "blocked", "tool": tool_name, "args": args,
                        "error": f"Shelter {args['shelter_id']} not found", "result": None}
            resources = db.scalars(select(Resource).where(Resource.shelter_id == shel.id)).all()
            return {"status": "executed", "tool": tool_name, "args": args,
            ...
            db.add(assignment)
            log = AgentActionLog(
                action_type="create_rescue_assignment",
                risk_classification="high",
                status="pending",
                payload=json.dumps({"request_id": args["request_id"],
                                    "volunteer_id": args["volunteer_id"],
                                    "shelter_id": args["shelter_id"]}),
            )
            db.add(log)
            ...
            log = AgentActionLog(
                ...
                status="executed",
                payload=json.dumps({"request_id": args["request_id"], "new_status": new_status}),
            )
            db.add(log)
            db.commit()
            return {"status": "executed", "tool": tool_name, "args": args,
            ...
            if not shel:
                return {"status": "blocked", "tool": tool_name, "args": args,
                        "error": f"Shelter {args['shelter_id']} not found", "result": None}
            res = db.scalar(select(Resource).where(
            ...
            log = AgentActionLog(
                action_type="reserve_relief_resources",
                risk_classification="high",
                status="executed",
                payload=json.dumps({"shelter_id": args["shelter_id"],
            ...
            log = AgentActionLog(
                ...
                risk_classification="high",
                status="pending",
                payload=json.dumps({"recipient_type": rtype,
                                    "recipient_id": rid,
                                    "channel": channel,
                                    "message": message}),
            )
...
def run_tool_from_call(tool_call) -> ExecutorResult:
